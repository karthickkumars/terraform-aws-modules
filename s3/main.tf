...
resource "aws_s3_bucket_policy" "this" {
  count = var.enable && local.attach_policy ? 1 : 0
...
data "aws_elb_service_account" "this" {
  count = var.enable && var.attach_elb_log_delivery_policy ? 1 : 0
}
...
data "aws_iam_policy_document" "lb_log_delivery" {
  count = var.enable && var.attach_lb_log_delivery_policy ? 1 : 0
...
data "aws_iam_policy_document" "deny_insecure_transport" {
  count = var.enable && var.attach_deny_insecure_transport_policy ? 1 : 0
...
resource "aws_s3_bucket_public_access_block" "this" {
  count = var.enable && var.attach_public_policy ? 1 : 0
...
resource "aws_s3_bucket_ownership_controls" "this" {
  count = var.enable && var.control_object_ownership ? 1 : 0
